...
- **Fields used → schema mapping**:
  ...
  | `school.school_url` | `schools.website_url` — the institution's own official site, normalized to always include a protocol (Scorecard returns bare domains like `"www.princeton.edu/"` for some schools) |
  | `school.price_calculator_url` | `schools.net_price_calculator_url` — the federally mandated Net Price Calculator every US Title-IV school must publish; a personalized, official "what will I actually pay after aid" tool, not this app's own estimate |
...
- **website_url/net_price_calculator_url/avg_tuition propagation**: `website_url`/`net_price_calculator_url` only ever come from Scorecard, written directly onto that college_scorecard row. A sibling row for the same institution from a different source (most commonly `qs_rankings` — QS's global-elite list heavily overlaps Scorecard's US coverage, e.g. Princeton/MIT/Stanford) doesn't get its own copy from Scorecard's response, but `findCandidateSchools`' de-duplication can end up picking either row as the actual candidate depending on world_rank/admission_rate — in practice a qs_rankings row usually wins for exactly these elite schools, since it has a world_rank and the college_scorecard row doesn't. `schoolRepository.propagateSchoolLinks()` writes the same values onto every sibling row sharing a (country, name) — matching a trailing parenthetical abbreviation the same way `enrichSchoolAcademicStats` already does for `sat_avg`/`hs_gpa_avg` — so whichever row wins the de-dup already carries them directly, called once per record right after `sync-schools.js`'s own `upsertSchoolFromSource` call. `avg_tuition` gets the same propagation (via `COALESCE`, since unlike the two link fields it can also legitimately come from US News — this only fills a sibling row's `avg_tuition` when that row doesn't already have one of its own, never overwrites).
