...
- **Why this exists alongside College Scorecard/QS**: College Scorecard and QS both give institution-level admission_rate/world_rank, which `admissionFitEngine.js` turns into a selectivity *band* (e.g. "QS top 20 → threshold 90") — a reasonable estimate, but still a band shared by every school in that tier. This export instead gives each school's own reported incoming-class average SAT (`sat_avg`) and HS GPA (`hs_gpa_avg`), so the fit engine can compare a student's academic index directly against the specific school's own numbers instead of a tier-wide estimate — see `admissionFitEngine.js`'s `selectivityFromSchoolAverages`.
