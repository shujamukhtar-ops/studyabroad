...
| Method | Path | Auth | Tier | Description |
|---|---|---|---|---|
| GET | `/api/matches` | required | any (both tiers get real matches) | School matches for caller's profile, each tagged with a `fit_category` (`Reach`/`Target`/`Safety`) and ordered by `rank_position` — the caller's *closest profile matches first*, computed by comparing the student's academic (+ premium: holistic) index against each school's own reported profile (US News per-school average SAT/GPA, when available — see DATA_SOURCES.md; falls back to a QS-rank/admission-rate-derived estimate for schools US News doesn't cover, i.e. every non-US school) via `ai-engine/admissionFitEngine.js`'s `computeFit`/`profileDistance`. Basic: judged on academic profile only (GPA + test scores), a 15-school shortlist balanced across Reach/Target/Safety, deterministic templated reasoning, no AI provider call. Premium: additionally blends in the Achievements-tab holistic score, a wider closest-match candidate pool, and AI-generated per-school reasoning. Each match also carries, when known for that school: `school_website_url` (official site — a student's actual path to apply), `school_net_price_calculator_url` (the school's own personalized net-price-after-aid tool, College Scorecard's federally mandated Net Price Calculator for US schools — see DATA_SOURCES.md), `school_avg_tuition` (sticker price), `school_net_price_after_aid` (US News's reported average net price actually paid by aid recipients), and `school_pct_receiving_aid` (context for that average). Any of these is `null` when not known for that school — the frontend falls back to a live search link rather than guessing a URL (same pattern as scholarship cards with no `source_url`). Response is `{schools, tier, upgradeNote?}` — `upgradeNote` is present (a short, non-blocking upsell string) only for basic-tier callers. Scholarships are a separate endpoint (below), not part of this response. |
| GET | `/api/scholarships` | required | any | Candidate scholarships for the caller's profile, filtered by `destination_countries` (which country the award funds study *in* — matched against the profile's `target_countries`), `eligible_nationalities` (which country the applicant is *from* — matched against the account's `home_country`), `degree_levels` (which degree level the award funds — matched against the profile's `degree_level`; e.g. a PhD-only scholarship like the Vanier Canada Graduate Scholarship is hidden from an undergraduate applicant, and vice versa), and major. On every one of these axes, an empty value on the scholarship side (or an unset `degree_level` on the profile) means open/unrestricted, never a guessed exclusion — see `backend/src/data-ingestion/sources/scholarships-worldwide.js` for how the archive's freeform `eligibility` text is parsed into `degree_levels` (only literal "Bachelor"/"Master"/"Phd" tokens map to a value; a non-degree "Course" award or unrecognized text stays unrestricted rather than being guessed into a level it doesn't state). Not tiered — this is filtered data, not AI ranking. Response: `{scholarships}`, each with `amount`/`amount_text` (best-effort number + the original readable award text), `deadline` (`'YYYY-MM-DD'` or `null` for rolling/no fixed deadline — never returns an ISO datetime), and `eligibility_note`. |
